/*
    ...
    暴力解法的时间复杂度为 O(N^3)（这里 N 是输入字符串的长度，枚举所有子串 O(N^2)，对于每一个子串计算最多出现的字符 O(N)。
    而题目的提示告诉我们字符串长度和 k 不会超过 10^4，暴力算法在这个数据规模下会超时。
    ...
    右边界先向右和移动，直到它不能移动了为止，左边界再继续向右移动，整个过程像极了一个滑动的窗口在一条线段上移动。
    ...
    复杂度分析：
    时间复杂度：O(N)，这里 NN 是输入字符串 S 的长度；
    空间复杂度：O(A)，这里 A 是输入字符串 S 出现的字符 ASCII 值的范围（感谢用户 @hyponarch 指正）。
    ...
    在 left++ 以后，由于可以不维护 maxCount 的定义，right - left > maxCount + k 不成立。
    因此 if 里面的代码块只会被执行一次。
    
    4. 可以不用一直用 res 记录滑动窗口的最大长度，最后返回 right - left 即可。
    答：依然是 #我们只关心最长替换 k 次以后重复子串的长度#，并且 maxCount 只会增加不会减少。
    在退出内层 if 语句的时候，区间 [left, right) 不一定是符合要求的子串，但是子串的长度一定等于题目要求的替换 k 次以后字符全都相等的最长子串。
    ...
    这一类问题的思考路径通常是：先思考暴力解法，分析暴力解法的缺点（一般而言暴力解法的缺点是重复计算），
    然后 #结合问题的特点#，使用「双指针」技巧对暴力解法进行剪枝。
    因此，#思考算法设计的合理性是更关键的，这一点适用于所有算法问题#。
